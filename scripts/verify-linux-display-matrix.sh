...
usage() {
  cat <<'USAGE'
usage: scripts/verify-linux-display-matrix.sh --app /path/to/LCDiff [--sample /path/to/sample.jar] [--output-dir platform-validation] [--no-launch]
...
launch_app() {
  ...
  if [[ "$mode" == "xwayland" ]]; then
    LCDIFF_FORCE_XWAYLAND=1 scripts/launch-linux-xwayland.sh "$APP" &
  else
    "$APP" &
  fi
  app_pid="$!"
  printf 'Launched LCDiff (%s) with pid %s. Use the window for the checks below.\n' "$mode" "$app_pid"
  sleep 2
}
...
REPORT
  launch_app "xwayland"
  printf '\nWayland detected. XWayland fallback checks with LCDIFF_FORCE_XWAYLAND=1.\n'
