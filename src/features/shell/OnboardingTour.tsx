...
export function hasSeenOnboarding(mode: Mode): boolean {
  try {
    return localStorage.getItem(onboardingKeyForMode(mode)) !== null;
  } catch {
    return false;
  }
}

function markOnboardingSeen(mode: Mode): void {
  try {
    localStorage.setItem(onboardingKeyForMode(mode), "seen");
  } catch {
    // The tour must remain dismissible when persistence is unavailable.
  }
}
