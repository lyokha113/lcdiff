...
describe("recordSession", () => {
  ...
  it("still returns the in-memory history when persistence is unavailable", () => {
    const setItem = vi.spyOn(Storage.prototype, "setItem").mockImplementation(() => {
      throw new DOMException("storage blocked", "SecurityError");
    });

    expect(recordSession("single", ["a.jar"], 1000)).toEqual([
      {
        id: entryKey("single", ["a.jar"]),
        mode: "single",
        paths: ["a.jar"],
        openedAt: 1000,
      },
...
describe("clearHistory", () => {
  ...
  it("does not throw when localStorage removal is unavailable", () => {
