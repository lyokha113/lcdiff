...
| Search | Monaco T1 find, Rust T2 path/text/constant-pool search without binary payload inflation, backend constant-pool match-kind regression, cached T3 deep source search on a dedicated background JVM worker, left/right/both scope, tagged clickable streaming result list with match kind and text/source line numbers, per-entry decompile-error skip, progress, cancel-token checks, and preemptive sidecar worker stop |
| UI design | Redesigned into a "precision developer instrument" aesthetic (graphite canvas + warm brass accent, atmospheric layered background, self-hosted distinctive type: Bricolage Grotesque wordmark, JetBrains Mono technical surfaces, Geist UI via `@fontsource-variable/*` so it renders offline). Functionality and configuration are split into labelled zones — Sources / Find / View and the diff workspace are operational, while the Decompiler-and-diff options sit behind a dashed divider and Mode lives in the top command bar — resolving the prior functionality/config muddle. All render and invariant DOM contracts (roles, names, classes, shadcn composition) preserved |
| Frontend stack | React shell now uses shadcn/ui source components (`Button`, `Input`, `Select`, `Checkbox`, `Badge`, `ContextMenu`, `Resizable`, `Dialog`, `Tooltip`) with Tailwind v4, `components.json`, `@` alias, and `cn()` helper; tree/editor panels use shadcn `ResizablePanelGroup`, tree-row merge actions use shadcn `ContextMenu`, signed-save confirmation uses shadcn `Dialog`, action explanations use shadcn `Tooltip`, and the frontend invariant verifier checks the shadcn/Tailwind config and component composition; Playwright render verifier boots the Vite shell, fails on browser page errors, checks the visible `jdiff` shell controls, and uses a browser IPC mock to prove the compare open/select/copy flow renders the pending target and pending row badge, then requires the signed-JAR warning Dialog before a confirmed Save anyway commits with a signed-invalidated message |
...
| Packaging code | Active Tauri bundle config with app icon/resources, per-arch release workflow with optional macOS Developer ID signing/notarization and Windows Authenticode signing steps, macOS `.app` plus DMG packaged from the final signed/notarized app when available, Linux AppImage/deb/rpm, Windows NSIS/MSI, Linux XWayland launch fallback helper, Linux display-matrix validation runner, Windows platform validation runner, macOS distribution validation runner, remote release workflow validation runner, macOS inside-out signing helper with default WebView/JVM entitlements, deterministic signed-app output, notarytool/stapler helper, and macOS DMG packaging helper |
...
| Remote package workflow | `.github/workflows/ci.yml` and `.github/workflows/release.yml` exist and satisfy `verify:ci-workflow` and `verify:release-workflow` (so `rtk npm run verify:all` passes end-to-end), including the macOS/Linux/Windows CI matrix, per-arch release bundles, conditional macOS signing/notarization, DMG packaging from the final macOS app, and Windows Authenticode signing; `scripts/verify-remote-release-workflow.sh` dispatches or inspects release runs and records artifact evidence; repository has no configured remote runner evidence in this workspace; pass criteria are in `docs/PLATFORM_VALIDATION.md` |
| macOS distribution | Ad-hoc inside-out signing with default WebView/JVM entitlements, deterministic signed-app output, signed/final app xattr cleanup, target-specific JLINK/app/JRE Mach-O architecture guards, post-DMG strict codesign verification, mounted-DMG `jdiff.app` verification, notarytool/stapler script syntax, conditional release-workflow wiring, bundled-JRE startup, and `scripts/verify-macos-distribution.sh` handoff with `platform-validation/macos-distribution-*.md` reporting, including `platform-validation/macos-distribution-aarch64-apple-darwin-20260605T133311Z.md` are verified locally; x86_64 local validation now fails fast when only an arm64 JLINK is present and requires `JDIFF_JLINK_X86_64_APPLE_DARWIN`; real Developer ID signing and notarization require credentials and `docs/PLATFORM_VALIDATION.md` |
