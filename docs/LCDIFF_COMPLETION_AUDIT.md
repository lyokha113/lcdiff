...
| Dirty state | Desktop state-machine and frontend interaction tests prove target lock, dirty file-switch rejection, clear unlock, signed-confirmation enforcement, guarded mode switches, stale async-result rejection, browser-safe Tauri effects, and Linux Wayland runtime drop-hint detection; window close asks before discarding staged copies |
...
| Frontend stack | React shell uses only the shadcn/ui source components it imports (`Button`, `Input`, `Select`, `Checkbox`, `Badge`, `ContextMenu`, `Popover`, `Dialog`, `Tooltip`) with Tailwind v4, native CSS motion, self-hosted Geist/JetBrains Mono fonts, `components.json`, `@` alias, and `cn()` helper. Vitest covers behavior; Playwright render verification fails on browser errors, exercises compare/open/search/merge/save flows, and checks `1280x800`, `1024x640`, and `720x520` for horizontal overflow and a visible workspace canvas. |
